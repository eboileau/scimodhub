...
def test_hub_config_from_dict():
    hub_cfg = hub_config_from_dict(CONFIG, "label")
...
def test_write_trackdb():
    with ExitStack() as stack:
        files = {
            ...
            for f in ["hub.txt", "genomes.txt", "description.html", "trackDb.txt"]
        }
        write_trackdb(SUBTRACKS, EXPECTED_HUB_CONFIG, files, "hg38")
    for k, v in files.items():
        assert v.final_content == EXPECTED_TRACK_DB[k]


def test_get_mouse_over():
    mouse_over = _get_mouse_over(HUB_CONFIG)
    assert mouse_over == (
