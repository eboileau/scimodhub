...
To download _chrom.sizes_ from Sci-ModoM, use
...
The `fetch` call downloads the [MODOMICS](https://www.genesilico.pl/modomics/modifications) codes (via the Sci-ModoM API) and writes them to a file _modomics.json_ under `working_dir`
...
The codes are used to build the track names. If this file is not present _e.g._ if `fetch` was not called, the `build` call will build track names using the MODOMICS short names.
...
- The list of datasets is filtered _on the fly_ for each organism; the API endpoint does not currently allow query parameters.
